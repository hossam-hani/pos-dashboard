...
class StocksServices {
  ...
  static Future<Stock> saveStock(
      {String productId, String storeId, String quantity, String supplierId, String id}) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    Account currentUser = Account.fromJson(jsonDecode(prefs.getString("account")));
    ...
    try {
      ...
      print({"product_id": productId, "store_id": storeId, "supplier_id": supplierId, "quantity": quantity});
      Response response = await dio.post(id != null ? "$baseUrl/stocks/$id" : "$baseUrl/stocks",
          data: {"product_id": productId, "store_id": storeId, "supplier_id": supplierId, "quantity": quantity});
